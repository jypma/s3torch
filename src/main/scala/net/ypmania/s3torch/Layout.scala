...
enum Layout:
  ...
  private[s3torch] def toNative: torchNative.Layout = torchNative.Layout.valueOf(this.toString)

object Layout:
  private[s3torch] def fromNative(native: torchNative.Layout) = Layout.valueOf(native.toString)
