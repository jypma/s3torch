...
object Token64 {
  class Ops[T <: net.ypmania.s3torch.DType.Int64](val dType: T) extends DTypeOps[T, Token64[T]] {
    ...
    override def copyToArray(t: pytorch.Tensor, a: Array[S]) = t.createBuffer[LongBuffer].get(a.map(_.value))

    override def toValue(double: Double) = Token64(double.toLong).asInstanceOf[ScalaType]
