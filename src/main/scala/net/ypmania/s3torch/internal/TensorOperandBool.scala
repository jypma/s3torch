...
trait TensorOperandBool[S <: Tuple, T <: DType, V] {
  type Out
...
object TensorOperandBool {
  given scalar[S <: Tuple, T <: DType, V](using toScalar: FromScala.ToScalar[V]): TensorOperandBool[S, T, V] with {
    type Out = Tensor[S, DType.Bool.type]
  ...
  given tensor[S <: Tuple, T <: DType, S2 <: Tuple, T2 <: DType, R <: Tuple](using Broadcast[S, S2, R]): TensorOperandBool[S, T, Tensor[S2, T2]] with {
    type Out = Tensor[R, DType.Bool.type]
