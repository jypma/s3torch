...
object TensorOperandApply {
  given scalar[S <: Tuple, T <: DType, D <: Device, V](using ops: DTypeOps.Scalar[T, V]): TensorOperandApply[S, T, D, V] with {
    override def apply(t: Tensor[S, T, D], v: V, withScalar: (pytorch.Tensor, pytorch.Scalar) => Unit, withTensor: (pytorch.Tensor, pytorch.Tensor) => Unit) = withScalar(t.native, ops.fromScalar(v))
  }
