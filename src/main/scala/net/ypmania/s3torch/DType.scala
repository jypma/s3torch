// Copied from storch as experiment.
/*
...
/** A [[torch.DType]] is an object that represents the data type of a [[torch.Tensor]]. PyTorch has twelve different
  ...
  */
// format: on
sealed abstract class DType private[s3torch] ():
  private[s3torch] def toScalarType: ScalarType = this match
...
object DType:
  private[s3torch] def fromScalarType(t: ScalarType): DType = t.intern() match
    ...
    case _ => undefined // TODO implement new types for pytorch 2.7+
...
trait LowPriorityDefaults:
  given float16: Default[Float16] = new Default[Float16] { def dtype = net.ypmania.s3torch.float16 }
  given bfloat16: Default[BFloat16] = new Default[BFloat16] { def dtype = net.ypmania.s3torch.bfloat16 }
  given float64: Default[Float64] = new Default[Float64] { def dtype = net.ypmania.s3torch.float64 }
  given complex32: Default[Complex32] = new Default[Complex32] { def dtype = net.ypmania.s3torch.complex32 }
  given complex64: Default[Complex64] = new Default[Complex64] { def dtype = net.ypmania.s3torch.complex64 }
  given complex128: Default[Complex128] = new Default[Complex128] { def dtype = net.ypmania.s3torch.complex128 }
object Default extends LowPriorityDefaults:
  given float32: Default[Float32] = new Default[Float32] { def dtype = net.ypmania.s3torch.float32 }
...
@SerialVersionUID(0L)
final case class Complex[@specialized(Float, Double) T](real: T, imag: T)
...
private[s3torch] def promotedDType[A <: DType, B <: DType](a: A, b: B) = (a, b) match
...
private[s3torch] def derivedArangeType[Start <: ScalaType, End <: ScalaType, Step <: ScalaType](
    start: Start,
    end: End,
    step: Step
...
private[s3torch] type TypedBuffer[T <: ScalaType] <: Buffer = T match
