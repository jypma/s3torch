...
class TransformerSpec extends UnitSpec {
  ...
  case object QSeqLen extends Dim.Static[3L]
  case object KVSeqLen extends Dim.Static[3L]
  ...
  describe("Transformer.PositionalEncoding") {
    val posEnc = new transformer.PositionalEncoding(QSeqLen, 0.0)
  ...
  describe("FeedForward") {
    val ff = new transformer.FeedForward(0.0)
  ...
  describe("EncoderBlock") {
    ...
    it("should encode a tensor while applying a mask") {
      val in = Tensor.zeros(BatchSize, QSeqLen, DModel)
      in((0, 1, 0)) = 1.0
